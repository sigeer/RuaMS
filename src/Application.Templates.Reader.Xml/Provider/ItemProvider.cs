...
namespace Application.Templates.Reader.Xml.Provider
{
    public sealed class ItemProvider : AbstractGroupProvider<AbstractItemTemplate>, IItemProvider
    {
        ...
        {
            return LoadConsume(_resolver.ResolveGroup(Type).FirstOrDefault(x => x.EndsWith("0238.img.xml"))!).OfType<MonsterCardItemTemplate>().ToList();
        }

        public List<MasteryItemTemplate> GetAllSkillBook()
        {
            return LoadConsume(_resolver.ResolveGroup(Type).FirstOrDefault(x => x.EndsWith("0228.img.xml") || x.EndsWith("0229.img.xml"))!).OfType<MasteryItemTemplate>().ToList();
        }
        public List<ConsumeItemTemplate> GetAllConsume()
        {
            return _resolver.ResolveGroup(Type).Where(x => x.Contains("Consume")).SelectMany(x => LoadConsume(x)).OfType<ConsumeItemTemplate>().ToList();
        }
