...
public class MapNavigator
{
            ...
            else if (topPlatform.HasValue && !bottomPlatform.HasValue)
            {
                ...
                for (int pi = 0; pi < nav._platforms.Count; pi++)
                {
                    ...
                    if (jumpUp != null)
                    {
                        ...
                        nav._adj[jumpUp.Value.fromPlat].Add((jumpUp.Value.toPlat, EncodeJumpLinkIdx(jumpIdx)));
                    }
                }
            }
        ...
        for (int i = 0; i < platforms.Count; i++)
        {
            for (int j = 0; j < platforms.Count; j++)
            {
                ...
                if (conn != null)
                {
                    ...
                    int jumpIdx = nav._jumpEdges.Count - 1;
                    // linkIdx < 0 编码跳跃连接，解码见 DecodeJumpLinkIdx
                    nav._adj[conn.FromPlatform].Add((conn.ToPlatform, EncodeJumpLinkIdx(jumpIdx)));
                }
            }
        }
    ...
    public List<MoveAction>? FindPath(Point from, Point to)
    {
        ...
        // Quick check: both on same ladder — climb directly
        if (startInfo.OnLadder != null && endInfo.OnLadder != null
            && startInfo.LadderLinkIdx >= 0 && endInfo.LadderLinkIdx >= 0
            && startInfo.LadderLinkIdx == endInfo.LadderLinkIdx)
        {
            return new List<MoveAction> { new(MoveActionType.Climb, to) };
        }
        ...
        int? foundEnd = null;
        HashSet<int> endSet = new(endInfo.PlatformIndices.Where(i => i >= 0));
        if (endSet.Count == 0) return null;

        foreach (var sp in startInfo.PlatformIndices)
        {
            if (sp < 0) continue;
            parent[sp] = (-1, -1);
            // 起点平台可能在终点集中（梯子 A↔B 两端，终点在 A 上）
            if (endSet.Contains(sp))
            {
                foundEnd = sp;
                break;
            }
        ...
        for (int i = 0; i < platPath.Count - 1; i++)
        {
            ...
            if (linkIdx == null) continue;

            if (linkIdx >= 0)
            {
                // Ladder connection
                var link = _ladderLinks[linkIdx.Value];
            ...
            {
                // Jump connection
                var jump = _jumpEdges[DecodeJumpLinkIdx(linkIdx.Value)];
    ...
    //  linkIdx 编码
    //
    //  _adj 中每条边携带一个 linkIdx，含义如下：
    //    >= 0  → 梯子连接，值 = _ladderLinks 索引
    //    < 0   → 跳跃连接，编码为 EncodeJumpLinkIdx(jumpEdgeIndex)
    //            解码用 DecodeJumpLinkIdx(encoded) 还原 jumpEdgeIndex
    // ================================================================

    /// <summary>将跳跃边索引编码为邻接表的 linkIdx（负数）</summary>
    private static int EncodeJumpLinkIdx(int jumpEdgeIdx) => -(jumpEdgeIdx + 1);

    /// <summary>从邻接表的 linkIdx 还原跳跃边索引</summary>
    private static int DecodeJumpLinkIdx(int encoded) => -encoded - 1;
