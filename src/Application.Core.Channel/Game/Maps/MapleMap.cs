...
public class MapleMap : IMap
{
    ...
    private void spawnAndAddRangedMapObject(IMapObject mapobject, Action<ChannelClient>? packetbakery, Func<Player, bool>? condition = null)
    {
    ...
    private byte dropItemsFromMonsterOnMap(List<DropEntry> dropEntry, Point pos, byte d, float chRate, byte droptype, int mobpos, Player chr, Monster mob, short dropDelay)
    {
    ...
    public byte dropGlobalItemsFromMonsterOnMap(List<DropEntry> globalEntry, Point pos, byte d, byte droptype, int mobpos, Player chr, Monster mob, short dropDelay)
    {
    ...
    private void spawnDrop(Item idrop, Point dropPos, IMapObject dropper, Player chr, byte droptype, short questid, short dropDelay)
    {
    ...
    public void spawnMesoDrop(int meso, Point position, IMapObject dropper, Player owner, bool playerDrop, byte droptype, short dropDelay = 0)
    {
    ...
    public Dictionary<int, Player> getMapAllPlayers()
    {
        return characters;
    }
    ...
    {
        this.killMonster(mob, (Player?)getPlayers().ElementAtOrDefault(0), false);
    }

    public void killMonster(int mobId)
    {
        Player? chr = (Player?)getPlayers().ElementAtOrDefault(0);
    ...
    public void spawnItemDrop(IMapObject dropper, Player owner, Item item, Point pos, byte dropType, bool playerDrop)
    {
    ...
    private void activateItemReactors(MapItem drop, ChannelClient c)
    {
    ...
    private void addPartyMemberInternal(Player chr, int partyid)
    {
        if (partyid == -1)
        {
            return;
    ...
    private void removePartyMemberInternal(Player chr, int partyid)
    {
        if (partyid == -1)
        {
            return;
    ...
    public void addPartyMember(Player chr, int partyid)
    {
        chrLock.EnterWriteLock();
        try
        {
    ...
    public void removePartyMember(Player chr, int partyid)
    {
        chrLock.EnterWriteLock();
        try
        {
    ...
    */
    ...
    private void broadcastBossHpMessage(Monster mm, int bossHash, Player? source, Packet packet, double rangeSq, Point? rangedFrom)
    {
        chrLock.EnterReadLock();
        try
        {
    ...
    private void sendObjectPlacement(ChannelClient c)
    {
    ...
    public IReadOnlyCollection<Player> getCharacters()
    {
    ...
    private class ActivateItemReactor : AbstractRunnable
    {
        ...
        private IMap _map;
        public ActivateItemReactor(IMap map, MapItem mapitem, Reactor reactor, ChannelClient c)
        {
            _map = map;
    ...
    {
        List<Player> players = new(getCharacters());
    ...
    {
        List<Player> chars = new(getCharacters());
    ...
    {   // ayy lmao
        ...
        {
            monsterDamaged = (Player from, int trueDmg) =>
            {
                ht.addHp(trueDmg);
            },
