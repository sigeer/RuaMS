...
namespace Application.Core.Game.TheWorld
{
    ...
    public class AllPlayerStorage
    {
        ...
        public static List<Player> GetAllOnlinedPlayers()
        {
        ...
        public static Player? GetOrAddCharacterById(int id, int level = 0)
        {
            var m = CachedData.GetValueOrDefault(id);
            if (m != null && m.Level >= level)
                return m.Data;

            Player? player = CharacterManager.GetPlayerById(id, level > 0);
        ...
        public static Player GetOrAddCharacterByIdOrThrow(int id, int level = 0) => GetOrAddCharacterById(id, level) ?? throw new BusinessCharacterNotFoundException(id);

        public static List<Player> GetPlayersByIds(IEnumerable<int> idList, int level = 0)
        {
        ...
        public static Player? GetOrAddCharacterByName(string name, int level = 0)
        {
            var m = NamedCacheData.GetValueOrDefault(name);
            if (m != null && m.Level >= level)
                return m.Data;

            Player? player = CharacterManager.GetPlayerByName(name, level > 0);
        ...
        public static List<Player> GetPlayersByNames(IEnumerable<string> nameList, int level = 0)
        {
            List<Player> list = new();
    ...
    public record DataLevel(int Level, Player Data);
}
