namespace Application.Core.Game.TheWorld
{
    ...
    public class WorldPlayerStorage
    {
        ...
        /// </summary>
        /// <returns></returns>
        public ICollection<Player> GetAllOnlinedPlayers()
        {
        ...
        public List<Player> GetPlayersByIds(IEnumerable<int> idList, int level = 0)
        {
            return AllPlayerStorage.GetPlayersByIds(idList, level).Where(x => x.World == _world).ToList();
        }
        public List<Player> GetPlayersByNames(IEnumerable<string> nameList, int level = 0)
        {
