...
public partial class Player
{
    ...
    public void setClient(ChannelClient c)
    {
        this.Client = c;
    }
    ...
    public void LinkNewChannelClient(ChannelClient newClient)
    {
        RemoveWorldWatcher();
    ...
    public void partyOperationUpdate(ITeam party, List<Player>? exPartyMembers)
    {
    ...
    private static void addPartyPlayerDoor(Player target)
    {
    ...
    private static void removePartyPlayerDoor(ITeam party, Player target)
    {
        target.removePartyDoor(party);
    }


    private static void updatePartyTownDoors(ITeam party, Player target, Player? partyLeaver, List<Player> partyMembers)
    {
        if (partyLeaver != null)
        {
        ...
        if (partyMembers.Count > 0)
        {
            ...
            foreach (Player pchr in partyMembers)
            {
            ...
            {
                foreach (Player pchar in partyMembers)
                {
    ...
    public bool applyConsumeOnPickup(int itemId)
    {
        if (itemId / 1000000 == 2)
        {
            ...
            if (ii.isConsumeOnPickup(itemId))
            {
                ...
                {
                    List<Player> partyMembers = this.getPartyMembersOnSameMap();
    ...
    private void notifyRemoteChannel(ChannelClient c, int remoteChannel, int otherCid, BuddyList.BuddyOperation operation)
    {
    ...
    public void exportExcludedItems(ChannelClient c)
    {
        Dictionary<int, HashSet<int>> petExcluded = this.getExcluded();
        foreach (var pe in petExcluded)
        {
    ...
    public bool insertNewChar(CharacterFactoryRecipe recipe)
    {
        ...
        try
        {
            ...
            var dbModel = GlobalTools.Mapper.Map<Player, CharacterEntity>(this);
    ...
    public override void sendDestroyData(ChannelClient Client)
    {
        Client.sendPacket(PacketCreator.removePlayerFromMap(this.getObjectId()));
    }

    public override void sendSpawnData(ChannelClient Client)
    {
