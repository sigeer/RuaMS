...
public class MakerProcessor
{
    ...
    public static void makerAction(InPacket p, ChannelClient c)
    {
    ...
    public static int getMakerSkillLevel(Player chr)
    {
        return chr.getSkillLevel((chr.getJob().getId() / 1000) * 10000000 + 1007);
    }

    private static short getCreateStatus(ChannelClient c, MakerItemCreateEntry recipe)
    {
        if (recipe.isInvalid())
        {
            return -1;
    ...
    private static bool hasItems(ChannelClient c, MakerItemCreateEntry recipe)
    {
        foreach (var p in recipe.getReqItems())
        {
            int itemId = p.ItemId;
    ...
    private static bool addBoostedMakerItem(ChannelClient c, int itemid, int stimulantid, Dictionary<int, short> reagentids)
    {
