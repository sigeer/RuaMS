...
namespace Application.Core.Channel.Client
{
    public class ChannelClient : ClientBase
    {
        public AccountDto AccountEntity { get; set; }
        IPacketProcessor<ChannelClient> _packetProcessor;
        public EngineStorage ScriptEngines { get; set; } = new EngineStorage();

        public ChannelClient(long sessionId, IWorldChannel currentServer, IChannel nettyChannel, IPacketProcessor<ChannelClient> packetProcessor, ILogger<IClientBase> log)
        ...
        private void RemovePlayer(Player player, IWorld wserv, bool serverTransition)
        {
            try
            {
        ...
        private void RemovePartyPlayer(Player player, IWorld wserv)
        {
