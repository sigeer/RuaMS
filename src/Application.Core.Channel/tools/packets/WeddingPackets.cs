...
{
    private static ILogger log = LogFactory.GetLogger("WeddingPackets");
    ...
    */
     ...
     * @return mplew
     */
    public static Packet OnMarriageResult(int marriageId, Player chr, bool wedding)
    {
    ...
    public static Packet onWeddingGiftResult(byte mode, List<string> itemnames, List<Item>? items)
    {
        ...
        switch (mode)
        {
            ...
            default:
                {
                    log.Warning("Unknown Wishlist Mode: {Mode}", mode);
                    break;
                }
        }
        return p;
