...
namespace Application.Core.scripting.npc
{
    ...
    public class TempConversation : NPCConversationManager
    {
        ...
        private TempConversation(ChannelClient c, int npc = NpcId.MAPLE_ADMINISTRATOR) : base(c, npc, null)
        {
        }
        ...
        /// <returns></returns>
        public static TempConversation? Create(ChannelClient c, int npc = NpcId.MAPLE_ADMINISTRATOR, bool force = true)
        {
            if (force)
        ...
        public static bool TryCreate(ChannelClient c, out TempConversation? data, int npc = NpcId.MAPLE_ADMINISTRATOR, bool force = true)
        {
            data = null;
            if (force)
                c.NPCConversationManager?.dispose();
