...
public class NPCScriptManager : AbstractScriptManager
{
    ...
    readonly EngineStorate<ChannelClient> _scripts = new EngineStorate<ChannelClient>();
    ...
    public bool isNpcScriptAvailable(ChannelClient c, string fileName)
    {
        IEngine? engine = null;
        if (fileName != null)
        {
    ...
    public bool start(ChannelClient c, int npc, Player? chr)
    {
        return start(c, npc, -1, chr);
    }

    public bool start(ChannelClient c, int npc, int oid, Player? chr)
    {
        return start(c, npc, oid, null, chr);
    }

    public bool start(ChannelClient c, int npc, string? fileName, Player? chr)
    {
        return start(c, npc, -1, fileName, chr);
    }

    public bool start(ChannelClient c, int npc, int oid, string? fileName, Player? chr)
    {
        return start(c, npc, oid, fileName, chr, false, "cm");
    }

    public bool start(ChannelClient c, ScriptedItem scriptItem, Player? chr)
    {
        return start(c, scriptItem.getNpc(), -1, scriptItem.getScript(), chr, true, "im");
    }

    public void start(string filename, ChannelClient c, int npc, List<Player> chrs)
    {
        try
        {
    ...
    private bool start(ChannelClient c, int npc, int oid, string? fileName, Player? chr, bool itemScript, string engineName)
    {
        try
        {
            if (c.NPCConversationManager != null)
    ...
    public void action(ChannelClient c, sbyte mode, sbyte type, int selection)
    {
        var iv = _scripts[c];
        if (iv != null)
        {
