...
namespace Application.Core.net.server.coordinator.matchchecker.listener
{
    ...
    {
        private static Player? getChallenger(int leaderid, HashSet<Player> matchPlayers)
        {
            return matchPlayers.FirstOrDefault(x => x.getId() == leaderid);
        }
        public override void onMatchCreated(Player leader, HashSet<Player> nonLeaderMatchPlayers, string message)
        {
            ...
            List<Player> chrMembers = new();
            foreach (Player mpc in chr.getParty().getMembers())
            {
                if (mpc.IsOnlined)
                {
                    chrMembers.Add(mpc);
        ...
        public override void onMatchAccepted(int leaderid, HashSet<Player> matchPlayers, string message)
        {
        ...
        public override void onMatchDeclined(int leaderid, HashSet<Player> matchPlayers, string message)
        {
            var chr = getChallenger(leaderid, matchPlayers);
            chr.dropMessage(5, LanguageConstants.getMessage(chr, LanguageConstants.CPQChallengeRoomDenied));
        }

        public override void onMatchDismissed(int leaderid, HashSet<Player> matchPlayers, string message) { }
    }
}
