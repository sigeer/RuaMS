...
public class PartySearchStorage
{
    ...
    private Dictionary<int, Player> fetchRemainingPlayers()
    {
        List<PartySearchCharacter> players = getStorageList();
        Dictionary<int, Player> remainingPlayers = new(players.Count);
    ...
    public void updateStorage(ICollection<Player> echelon)
    {
        Dictionary<int, Player> newcomers = new();
        foreach (Player chr in echelon)
        {
            newcomers.AddOrUpdate(chr.getId(), chr);
        }

        Dictionary<int, Player> curStorage = fetchRemainingPlayers();
    ...
    public Player? callPlayer(int callerCid, int callerMapid, int minLevel, int maxLevel)
    {
