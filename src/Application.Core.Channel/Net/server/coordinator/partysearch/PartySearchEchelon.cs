...
public class PartySearchEchelon
{
    ...
    private Dictionary<int, WeakReference<Player>> echelon = new(20);

    public PartySearchEchelon()
    {
    }

    public List<Player> exportEchelon()
    {
        psLock.EnterWriteLock();     // reversing read/write actually could provide a lax yet sure performance/precision trade-off here
        try
        {
            List<Player> players = new(echelon.Count);

            foreach (WeakReference<Player> chrRef in echelon.Values)
            {
    ...
    public bool detachPlayer(Player chr)
    {
        psLock.EnterReadLock();
        try
        {
