...
public class PartySearchCoordinator
{
    ...
    private List<Player> leaderQueue = new();

    private Dictionary<int, Player> searchLeaders = new();
    private Dictionary<int, LeaderSearchMetadata> searchSettings = new();

    private Dictionary<Player, LeaderSearchMetadata> timeoutLeaders = new();
    ...
    private Player? fetchPlayer(int callerCid, int callerMapid, Job job, int minLevel, int maxLevel)
    {
        return storage.GetValueOrDefault(getPartySearchJob(job))!.callPlayer(callerCid, callerMapid, minLevel, maxLevel);
    }

    private void addQueueLeader(Player leader)
    {
    ...
    private void removeQueueLeader(Player leader)
    {
        leaderQueueLock.EnterReadLock();
        try
        {
    ...
    public void registerPartyLeader(Player leader, int minLevel, int maxLevel, int jobs)
    {
    ...
    private void registerPartyLeader(Player leader, LeaderSearchMetadata settings)
    {
        if (searchLeaders.ContainsKey(leader.getId()))
        {
            return;
    ...
    public void unregisterPartyLeader(Player leader)
    {
    ...
    private Player? searchPlayer(Player leader)
    {
    ...
    private bool sendPartyInviteFromSearch(Player chr, Player leader)
    {
        if (chr == null)
        {
            return false;
    ...
    private KeyValuePair<List<Player>, List<Player>> fetchQueuedLeaders()
    {
        List<Player> queuedLeaders, nextLeaders;
    ...
    private void registerLongTermPartyLeaders(List<KeyValuePair<Player, LeaderSearchMetadata>> recycledLeaders)
    {
        leaderQueueLock.EnterReadLock();
        try
        {
    ...
    private void unregisterLongTermPartyLeader(Player leader)
    {
        leaderQueueLock.EnterReadLock();
        try
        {
    ...
    {
        Dictionary<Player, LeaderSearchMetadata> timeoutLeadersCopy;
        leaderQueueLock.EnterWriteLock();
        try
        {
    ...
    public void runPartySearch()
    {
        ...
        List<Player> searchedLeaders = new();
        List<Player> recalledLeaders = new();
        List<Player> expiredLeaders = new();

        foreach (Player leader in queuedLeaders.Key)
        {
        ...
        foreach (Player leader in searchedLeaders)
        {
        ...
        List<KeyValuePair<Player, LeaderSearchMetadata>> recycledLeaders = new();
        foreach (Player leader in expiredLeaders)
        {
            searchLeaders.Remove(leader.getId());
