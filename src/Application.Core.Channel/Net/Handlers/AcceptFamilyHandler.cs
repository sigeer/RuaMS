...
public class AcceptFamilyHandler : ChannelHandlerBase
{
    ...
    public AcceptFamilyHandler(ILogger<AcceptFamilyHandler> logger)
    {
        _logger = logger;
    }
    ...
    private void insertNewFamilyRecord(int characterID, int familyID, int seniorID, bool updateChar)
    {
        try
        {
            ...
            {
                _logger.LogError(e, "Could not save new family record for chrId {CharacterId}", characterID);
            }
            if (updateChar)
            {
                try
                ...
                {
                    _logger.LogError(e, "Could not update 'characters' 'familyid' record for chrId {CharacterId}", characterID);
                }
            }
        }
        catch (Exception e)
        {
            _logger.LogError(e, "Could not get connection to DB while inserting new family record");
        }
    }
}
