...
namespace Application.Plugin.Script
{
    internal partial class NpcScript
    {
        ...
        public async Task party1_play()
        {
            ...
            if (eim.ClearedMaps.TryGetValue(curMap, out var s) && s == StageStatus.Completed)
            {
                if (stage < 5)
                {
            ...
            if (curMap == 103000800)
            {
                ...
                else
                {
                    ...
                    {
                        data = Random.Shared.Next(stage1.Quest.Length) + 1;   //data will be counted from 1
                        eim.gridInsert(getPlayer(), data);

                        var question = stage1.Quest[data - 1];
                        await SayNext(question);
                    }
                    else
                    {
                        var answer = stage1.Answer[data - 1];
                        ...
                        {
                            var question = stage1.Quest[eim.gridCheck(getPlayer()) - 1];
            ...
            {
                await CheckStage2_3_4(eim, curMap);
            }
        ...
        async Task CheckStage2_3_4(AbstractEventInstanceManager eim, int mapId)
        {
