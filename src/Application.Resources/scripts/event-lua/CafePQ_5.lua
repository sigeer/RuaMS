...
-- 创建自定义事件
local CafePQ5 = BaseEvent:extend()

function CafePQ5:setup(level, lobbyid)
    local eim = BaseEvent.setup(self, level, lobbyid)
    eim:setProperty("stage", 0)
    eim:setIntProperty("couponsNeeded", 500)
    return eim
end

function CafePQ5:clearPQ(eim)
...
-- 创建事件实例
local event = CafePQ5:new(config)
