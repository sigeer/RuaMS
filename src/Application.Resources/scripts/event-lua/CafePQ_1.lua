...
-- 创建自定义事件
local CafePQ1 = BaseEvent:extend()

function CafePQ1:setup(level, lobbyid)
    local eim = BaseEvent.setup(self, level, lobbyid)
    eim:setProperty("stage", 0)
    eim:setIntProperty("couponsNeeded", 400)
    return eim
end

function CafePQ1:clearPQ(eim)
...
-- 创建事件实例
local event = CafePQ1:new(config)
