...
-- 创建自定义事件
local CafePQ3 = BaseEvent:extend()

function CafePQ3:setup(level, lobbyid)
    local eim = BaseEvent.setup(self, level, lobbyid)
    eim:setProperty("stage", 0)
    eim:setIntProperty("couponsNeeded", 350)
    return eim
end

function CafePQ3:clearPQ(eim)
...
-- 创建事件实例
local event = CafePQ3:new(config)
