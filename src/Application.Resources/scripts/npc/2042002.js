        ...
        if (cm.getPlayer().getMapId() == 980000010) {
            ...
            if (status == 0) {
                if (cm.getChar().getParty() != null) {
                    ...
                    if (cm.getPlayer().getFestivalPoints() >= 300) {
                        ...
                        shiu += "#rC#k";
                        rnk = 30;
                        cm.sendOk("很遗憾，你要么平局要么失败了。胜利属于那些努力奋斗的人。我看到了你的努力，所以胜利离你并不遥远。继续加油！\r\n#b你的结果：" + shiu);
                    } else {
                        shiu += "#rD#k";
                        rnk = 40;
            ...
            if (status == 0) {
                if (cm.getChar().getParty() != null) {
                    ...
                    if (cm.getPlayer().getFestivalPoints() >= 300) {
                        ...
                        shi += "#rD#k";
                        rnk = 4;
                        cm.sendOk("恭喜你的胜利，尽管你的表现并没有完全体现出来。在下一次的怪物嘉年华中更加积极参与吧！\r\n\r\n#b你的结果：" + shi);
                    }
                } else {
                    cm.warp(980000000, 0);
                    cm.dispose();
            ...
            if (status == 0) {
                ...
                if (selection == 0) {
                    ...
                    cm.sendNext("“#b怪物嘉年华#k 由两组进入战场，并释放对方召唤的怪物组成。 #b通过获得嘉年华点数（CP）来确定胜利者的战斗团队#k。”");
                } else if (selection == 2) {
                ...
                if (select == 0) {
                    ...
                    cm.sendNext("进入嘉年华领域时，你的任务是通过击败来自对立阵营的怪物来获得CP，并使用这些CP来分散对立阵营的注意，防止他们攻击怪物。");
                } else if (select == 2) {
                    cm.sendNext("一旦你习惯了这些指令，试着使用 #bTAB 和 F1 ~ F12#k。#bTAB 可以在召唤怪物/使用技能/保护者之间切换#k，而 #bF1 ~ F12 可以直接访问窗口之一#k。");
                }
            } else if (status == 63) {
                if (select == 0) {
                    cm.sendNext("我知道你们用真正的武器互相战斗太危险了；我不会建议这样野蛮的行为。我的朋友，我所提供的是竞争的乐趣。战斗的激动和与如此强大和积极的人竞争的激动。我提议你们的团队和对立团队都召唤怪物，并打败对方团队召唤的怪物。这就是怪物嘉年华的精髓。此外，你可以使用在怪物嘉年华期间赚取的枫币来获得新物品和武器！");
                } else if (select == 1) {
                ...
                if (select == 1) {
                    cm.sendNext("哦，不用担心变成幽灵。在怪物嘉年华中，你死后不会损失经验。所以这真的是一种独特的体验！");
                    cm.dispose();
                } else if (select == 2) {
