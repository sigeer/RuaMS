        ...
        if (cm.getPlayer().getMapId() == 980000010) {
            ...
            if (status == 0) {
                ...
                if (selection == 0) {
                    ...
                    cm.sendNext("“#b怪物嘉年华#k 由两组进入战场，并释放对方召唤的怪物组成。 #b通过获得的嘉年华点数（CP）来确定胜利者的战斗团队#k。”");
                } else if (selection == 2) {
                ...
                if (select == 0) {
                    ...
                    cm.sendNext("一旦你习惯了这些指令，试着使用 #bTAB 和 F1 ~ F12#k。#bTAB 可以在召唤怪物/使用技能/保护者之间切换#k，而 #bF1 ~ F12 可以直接让你访问其中一个窗口#k。");
                }
            } else if (status == 63) {
                if (select == 0) {
                    ...
                    cm.sendNext("召唤怪物会召唤一个攻击对方队伍的怪物，并将其控制在其下。使用CP召唤一个被召唤的怪物，它将出现在同一区域，攻击对方团队。");
                }
            } else if (status == 64) {
                if (select == 0) {
