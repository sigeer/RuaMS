...
public class GeneralChatHandler : ChannelHandlerBase
{
    ...
    public override async Task HandlePacket(InPacket p, IChannelClient c)
    {
        ...
        else if (heading != '/')
        {
            ...
            {
                await chr.dropMessage(5, "The map you are in is currently muted. Please try again later.");
                return;
            }

            await chr.BroadcastMap(PacketCreator.getChatText(chr.getId(), s, chr.getWhiteChat(), show));
