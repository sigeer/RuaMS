...
public class ServerMessageController : TimelyControllerBase
{
    ...
    public ServerMessageController(IWorldChannel world) : base("", TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(10))
    {
        worldChannel = world;
    }
    ...
    protected override void HandleRun()
    {
        ...
        if (toRemove.Count > 0)
        {
            foreach (int chrid in toRemove)
            {
                ...
                {
                    chr.sendPacket(PacketCreator.serverMessage(worldChannel.ServerConfig.ServerMessage));
                }
            }
        }
    }
