...
public class OpcodeConstants
{
    ...
    public static void generateOpcodeNames()
    {
        ...
        {
            sendOpcodeNames.AddOrUpdate((int)op, op.ToString());
        }
        // WEDDING_TALK ”Î WEDDING_TALK_MORE œ‡Õ¨
        foreach (RecvOpcode op in Enum.GetValues<RecvOpcode>())
        {
            recvOpcodeNames.AddOrUpdate((int)op, op.ToString());
        }
    }
