...
public class Monster : AbstractLoadedLife
{
    private ILogger log ;
    ...
    private int mp;
    private WeakReference<Character?> controller = new (null);
    ...
    {   // roam: not casting attack or skill animations
        ...
        try
        {
            ...
            if (animationTime > 0)
            {
                ...
                return service.registerMobOnAnimationEffect(map.getId(), this.GetHashCode(), animationTime);
            }
            else
            {
                return true;
    ...
    public void broadcastMobHpBar(Character from)
    {
        ...
        else if (!isBoss())
        {
            ...
            {
                foreach (PartyCharacter mpc in from.getParty().getMembers())
                {
    ...
    public bool damage(Character attacker, int damage, bool stayAlive)
    {
        bool lastHit = false;
     ...
     * @param stayAlive
     */
    private void applyDamage(Character from, int damage, bool stayAlive, bool fake)
    {
    ...
    public void applyFakeDamage(Character from, int damage, bool stayAlive)
    {
    ...
    public bool isAttackedBy(Character chr)
    {
        return takenDamage.ContainsKey(chr.getId());
    }

    private static bool isWhiteExpGain(Character chr, Dictionary<int, float> personalRatio, double sdevRatio)
    {
    ...
    private void distributePlayerExperience(Character chr, float exp, float partyBonusMod, int totalPartyLevel, bool highestPartyDamager, bool whiteExpGain, bool hasPartySharers)
    {
    ...
    private void distributePartyExperience(Dictionary<Character, long> partyParticipation, float expPerDmg, HashSet<Character> underleveled, Dictionary<int, float> personalRatio, double sdevRatio)
    {
        ...
        {
            foreach (Character member in partyParticipation.Keys.First().getPartyMembersOnSameMap())
            {
        ...
        foreach (Character mc in expMembers)
        {
    ...
    private void distributeExperience(int killerId)
    {
        ...
        Dictionary<Party, Dictionary<Character, long>> partyExpDist = new();
        Dictionary<Character, long> soloExpDist = new();

        Dictionary<int, Character> mapPlayers = map.getMapAllPlayers();
        ...
        foreach (Dictionary<Character, long> m in partyExpDist.Values)
        {
            float ratio = 0.0f;
            foreach (var e in m)
            {
        ...
        foreach (Character mc in underleveled)
        {
    ...
    private float getStatusExpMultiplier(Character attacker, bool hasPartySharers)
    {
    ...
    private void giveExpToCharacter(Character attacker, float? personalExp, float? partyExp, bool white, bool hasPartySharers)
    {
        if (attacker.isAlive())
        {
            if (personalExp != null)
    ...
    public List<MonsterDropEntry> retrieveRelevantDrops()
    {
        ...
        Dictionary<int, Character> pchars = map.getMapAllPlayers();

        List<Character> lootChars = new();
    ...
    public Character? killBy(Character? killer)
    {
        ...
        if (toSpawn != null)
        {
            ...
            if (toSpawn.Count > 0)
            {
                ...
                {
                    Character controller = lastController.Key;
        ...
        var looter = map.getCharacterById(getHighestDamagerId());
    ...
    private void dispatchRaiseQuestMobCount()
    {
        ...
        {
            Dictionary<int, Character> mapChars = map.getMapPlayers();
            if (mapChars.Count > 0)
            {
                int mobid = getId();
    ...
    public bool applyStatus(Character from, MonsterStatusEffect status, bool poison, long duration)
    {
        return applyStatus(from, status, poison, duration, false);
    }

    public bool applyStatus(Character from, MonsterStatusEffect status, bool poison, long duration, bool venom)
    {
            ...
            try
            {
                foreach (MonsterStatus stat in statis.Keys)
                {
                    ...
                    if (oldEffect != null)
                    {
                        ...
                        {
                            MobStatusService serviced = (MobStatusService)map.getChannelServer().getServiceAccess(ChannelServices.MOB_STATUS);
    ...
    public void applyMonsterBuff(Dictionary<MonsterStatus, int> stats, int x, long duration, MobSkill skill, List<int> reflection)
    {
        ...
        MonsterStatusEffect effect = new MonsterStatusEffect(stats, null, skill, true);
    ...
    public class DamageTask : AbstractRunnable
    {
        ...
        public DamageTask(Monster mapleMonster, int dealDamage, Character chr, MonsterStatusEffect status, int type)
        {
    ...
    public bool isCharacterPuppetInVicinity(Character chr)
    {
    ...
    private Character? getNextControllerCandidate()
    {
        ...
        foreach (Character chr in getMap().getAllPlayers())
        {
            if (!chr.isHidden())
            {
     ...
     */
    public KeyValuePair<Character, bool> aggroRemoveController()
    {
     ...
     * player controller.
     */
    public void aggroSwitchController(Character? newController, bool immediateAggro)
    {
    ...
    {
        var mmac = map.getAggroCoordinator();
    ...
    private void aggroUpdatePuppetController(Character? newController)
    {
        ...
        if (newController == null || !isCharacterPuppetInVicinity(newController))
        {
            ...
            {
                var chr = map.getCharacterById(cid);
     ...
     */
    public bool? aggroMoveLifeUpdate(Character player)
    {
    ...
    private static void aggroMonsterControl(Client c, Monster mob, bool immediateAggro)
    {
    ...
    private void aggroRefreshPuppetVisibility(Character chrController, Summon puppet)
    {
        ...
        foreach (Monster mob in chrController.getControlledMonsters())
        {
