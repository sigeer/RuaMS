...
public class PlayerNPC : AbstractMapObject
{
    ...
    public static bool canSpawnPlayerNpc(string name, int mapid)
    {
        ...
        {
            using var dbContext = new DBContext();
            return !dbContext.Playernpcs.Where(x => EF.Functions.Like(x.Name, name) && x.Map == mapid).Any();
        }
        catch (Exception e)
        {
            Log.Logger.Error(e.ToString());
    ...
    private static List<int> removePlayerNPCInternal(MapleMap? map, Character chr)
    {
        ...
        {
            using var dbContext = new DBContext();
            var queryExpression = dbContext.Playernpcs.Where(x => EF.Functions.Like(x.Name, chr.getName()));
            if (map != null)
    ...
    private static KeyValuePair<PlayerNPC?, List<int>> processPlayerNPCInternal(MapleMap map, Point? pos, Character chr, bool create)
    {
        lock (proLock)
        {
            if (create)
    ...
    public static bool spawnPlayerNPC(int mapid, Character chr)
    {
        return spawnPlayerNPC(mapid, null, chr);
    }

    public static bool spawnPlayerNPC(int mapid, Point? pos, Character chr)
    {
        if (chr == null)
        {
            return false;
        ...
        if (pn != null)
        {
            foreach (Channel channel in Server.getInstance().getChannelsFromWorld(chr.getWorld()))
            {
    ...
    private static PlayerNPC getPlayerNPCFromWorldMap(string name, int world, int map)
    {
        ...
        foreach (MapObject pnpcObj in wserv.getChannel(1).getMapFactory().getMap(map).getMapObjectsInRange(new Point(0, 0), double.PositiveInfinity, Arrays.asList(MapObjectType.PLAYER_NPC)))
        {
    ...
    public static void removePlayerNPC(Character chr)
    {
        ...
        foreach (int mapid in updateMapids)
        {
            ...
            if (pn != null)
            {
                foreach (Channel channel in Server.getInstance().getChannelsFromWorld(worldid))
                {
                    MapleMap m = channel.getMapFactory().getMap(mapid);
    ...
    public static void multicastSpawnPlayerNPC(int mapid, int world)
    {
        ...
        foreach (Character mc in wserv.loadAndGetAllCharactersView())
        {
            mc.setClient(c);
    ...
    public static void removeAllPlayerNPC()
    {
        try
        {
            ...
            foreach (var item in dataList)
            {
                ...
                foreach (Channel channel in Server.getInstance().getChannelsFromWorld(world))
                {
                    MapleMap m = channel.getMapFactory().getMap(map);

                    foreach (MapObject pnpcObj in m.getMapObjectsInRange(new Point(0, 0), double.PositiveInfinity, Arrays.asList(MapObjectType.PLAYER_NPC)))
                    {
                        PlayerNPC pn = (PlayerNPC)pnpcObj;
