...
public class MapleMap
{
    ...
    private Dictionary<int, MapObject> mapobjects = new();
    ...
    private HashSet<Character> characters = new();
    ...
    private List<WeakReference<MapObject>> registeredDrops = new();
    ...
    private void broadcastPacket(Packet packet, Func<Character, bool> chrFilter)
    {
    ...
    public List<MapObject> getMapObjectsInRect(Rectangle box, List<MapObjectType> types)
    {
        ...
        List<MapObject> ret = new();
        try
        {
            foreach (MapObject l in mapobjects.Values)
            {
    ...
    {
        foreach (MapObject o in getMapObjects())
        {
    ...
    public void limitReactor(int rid, int num)
    {
        ...
        foreach (MapObject obj in getReactors())
        {
    ...
    {
        foreach (MapObject mo in getReactors())
        {
    ...
    {
        foreach (Character chr in this.getCharacters())
        {
            if (chr.getPartyId() != -1)
            {
    ...
    private void spawnAndAddRangedMapObject(MapObject mapobject, Action<Client>? packetbakery)
    {
        spawnAndAddRangedMapObject(mapobject, packetbakery, null);
    }

    private void spawnAndAddRangedMapObject(MapObject mapobject, Action<Client>? packetbakery, Func<Character, bool>? condition)
    {
        List<Character> inRangeCharacters = new();
        int curOID = getUsableOID();
        ...
        try
        {
            ...
            {
                if (condition == null || (condition?.Invoke(chr) ?? false))
                {
        ...
        foreach (Character chr in inRangeCharacters)
        {
    ...
    private void dropFromMonster(Character chr, Monster mob, bool useBaseRate)
    {
    ...
    public void dropItemsFromMonster(List<MonsterDropEntry> list, Character chr, Monster mob)
    {
        if (mob.dropsDisabled() || !dropsOn)
        {
            return;
    ...
    public void dropFromFriendlyMonster(Character chr, Monster mob)
    {
        dropFromMonster(chr, mob, true);
    }

    public void dropFromReactor(Character chr, Reactor reactor, Item drop, Point dropPos, short questid)
    {
    ...
    private void spawnDrop(Item idrop, Point dropPos, MapObject dropper, Character chr, byte droptype, short questid)
    {
        MapItem mdrop = new MapItem(idrop, dropPos, dropper, chr, chr.getClient(), droptype, false, questid);
    ...
    public void spawnMesoDrop(int meso, Point position, MapObject dropper, Character owner, bool playerDrop, byte droptype)
    {
        Point droppos = calcDropPos(position, position);
        MapItem mdrop = new MapItem(meso, droppos, dropper, owner, owner.getClient(), droptype, playerDrop);
        mdrop.setDropTime(Server.getInstance().getCurrentTime());
        ...
        spawnAndAddRangedMapObject(mdrop, c =>
        {
            ...
            try
            {
                c.sendPacket(PacketCreator.dropItemFromMapObject(c.getPlayer(), mdrop, dropper.getPosition(), droppos, 1));
            }
            finally
            {
                mdrop.unlockItem();
    ...
    public void disappearingItemDrop(MapObject dropper, Character owner, Item item, Point pos)
    {
        Point droppos = calcDropPos(pos, pos);
        MapItem mdrop = new MapItem(item, droppos, dropper, owner, owner.getClient(), 1, false);
    ...
    public void disappearingMesoDrop(int meso, MapObject dropper, Character owner, Point pos)
    {
        Point droppos = calcDropPos(pos, pos);
        MapItem mdrop = new MapItem(meso, droppos, dropper, owner, owner.getClient(), 1, false);
    ...
    {
        int count = 0;
        foreach (MapObject m in getMapObjectsInRange(new Point(0, 0), double.PositiveInfinity, Arrays.asList(MapObjectType.MONSTER)))
        {
    ...
    public List<MapObject> getReactors()
    {
    ...
    public Dictionary<int, Character> getMapAllPlayers()
    {
        Dictionary<int, Character> pchars = new();
        foreach (Character chr in this.getAllPlayers())
        {
    ...
    public bool damageMonster(Character chr, Monster monster, int damage)
    {
        if (monster.getId() == MobId.ZAKUM_1)
        {
            foreach (MapObject mapObj in chr.getMap().getMapObjects())
            {
    ...
    public void killMonster(Monster monster, Character? chr, bool withDrops)
    {
        killMonster(monster, chr, withDrops, 1);
    }

    public void killMonster(Monster monster, Character? chr, bool withDrops, int animation)
    {
        if (monster == null)
        {
            return;
            ...
            if (removeKilledMonsterObject(monster))
            {
                try
                {
                    ...
                    if (buff > -1)
                    {
                        ...
                        foreach (MapObject mmo in this.getPlayers())
                        {
                    ...
                    if (MobId.isZakumArm(monster.getId()))
                    {
                        ...
                        ICollection<MapObject> objects = getMapObjects();
                    ...
                    if (monster.hasBossHPBar())
                    {
                        foreach (Character mc in this.getAllPlayers())
                        {
    ...
    {
        this.killMonster(mob, (Character?)getPlayers().ElementAtOrDefault(0), false);
    }

    public void killMonster(int mobId)
    {
        Character? chr = (Character?)getPlayers().ElementAtOrDefault(0);
    ...
    {
        Dictionary<int, Character> mapChars = this.getMapPlayers();

        if (mapChars.Count > 0)
        {
            Character defaultChr = mapChars.FirstOrDefault().Value;
            List<Monster> mobList = getAllMonsters();
    ...
    public void softKillAllMonsters()
    {
        ...
        foreach (MapObject monstermo in getMapObjectsInRange(new Point(0, 0), double.PositiveInfinity, Arrays.asList(MapObjectType.MONSTER)))
        {
    ...
    public void destroyReactors(int first, int last)
    {
        ...
        foreach (MapObject obj in reactors)
        {
            Reactor mr = (Reactor)obj;
    ...
    {
        List<Point> points = new();
        List<MapObject> listObjects = new();
        List<MapObject> targets = new();
        ...
        foreach (MapObject obj in listObjects)
        {
            Reactor mr = (Reactor)obj;
    ...
    private Dictionary<int, MapObject> getCopyMapObjects()
    {
        objectLock.EnterReadLock();
        try
        {
    ...
    {
        foreach (MapObject obj in getMapObjects())
        {
    ...
    {     // assumption: there's at most one of the same NPC in a map.
        List<MapObject> npcs = getMapObjectsInRange(new Point(0, 0), double.PositiveInfinity, Arrays.asList(MapObjectType.NPC));

        chrLock.EnterReadLock();
        objectLock.EnterWriteLock();
        try
        {
            foreach (MapObject obj in npcs)
            {
                if (((NPC)obj).getId() == npcid)
                {
    ...
    public void spawnMist(Mist mist, int duration, bool poison, bool fake, bool recovery)
    {
        ...
        if (poison)
        {
            Action poisonTask = () =>
            {
                ...
                foreach (MapObject mo in affectedMonsters)
                {
        ...
        else if (recovery)
        {
            Action poisonTask = () =>
            {
                ...
                foreach (MapObject mo in players)
                {
                    if (mist.makeChanceResult())
                    {
    ...
    public void spawnItemDrop(MapObject dropper, Character owner, Item item, Point pos, bool ffaDrop, bool playerDrop)
    {
        spawnItemDrop(dropper, owner, item, pos, (byte)(ffaDrop ? 2 : 0), playerDrop);
    }

    public void spawnItemDrop(MapObject dropper, Character owner, Item item, Point pos, byte dropType, bool playerDrop)
    {
        ...
        Point droppos = calcDropPos(pos, pos);
        MapItem mdrop = new MapItem(item, droppos, dropper, owner, owner.getClient(), dropType, playerDrop);
        mdrop.setDropTime(Server.getInstance().getCurrentTime());
    ...
    public void spawnItemDropList(List<int> list, MapObject dropper, Character owner, Point pos)
    {
        spawnItemDropList(list, 1, 1, dropper, owner, pos, true, false);
    }

    public void spawnItemDropList(List<int> list, int minCopies, int maxCopies, MapObject dropper, Character owner, Point pos)
    {
    ...
    public void spawnItemDropList(List<int> list, int minCopies, int maxCopies, MapObject dropper, Character owner, Point pos, bool ffaDrop, bool playerDrop)
    {
    ...
    private void activateItemReactors(MapItem drop, Client c)
    {
        Item item = drop.getItem();

        foreach (MapObject o in getReactors())
        {
    ...
    private static void announcePlayerDiseases(Client c)
    {
    ...
    */
     ...
     */
    public void broadcastMessage(Character? source, Packet packet, bool repeatToSource)
    {
     ...
     * @param ranged
     */
    public void broadcastMessage(Character? source, Packet packet, bool repeatToSource, bool ranged)
    {
    ...
    public void broadcastSpawnPlayerMapObjectMessage(Character source, Character player, bool enteringField)
    {
        broadcastSpawnPlayerMapObjectMessage(source, player, enteringField, false);
    }

    public void broadcastGMSpawnPlayerMapObjectMessage(Character source, Character player, bool enteringField)
    {
        broadcastSpawnPlayerMapObjectMessage(source, player, enteringField, true);
    }

    private void broadcastSpawnPlayerMapObjectMessage(Character source, Character player, bool enteringField, bool gmBroadcast)
    {
        chrLock.EnterReadLock();
        try
        {
    ...
    private void sendObjectPlacement(Client c)
    {
        ...
        if (chr != null)
        {
            foreach (MapObject o in getMapObjectsInRange(chr.getPosition(), getRangedDistance(), rangedMapobjectTypes))
            {
                if (o.getType() == MapObjectType.REACTOR)
                {
    ...
    public List<MapObject> getMapObjectsInRange(Point from, double rangeSq, List<MapObjectType> types)
    {
        ...
        try
        {
            foreach (MapObject l in getMapObjects())
            {
                if (types.Contains(l.getType()))
                {
    ...
    public Dictionary<int, Character> getMapPlayers()
    {
        chrLock.EnterReadLock();
        try
        {
    ...
    public IReadOnlyCollection<Character> getCharacters()
    {
        chrLock.EnterReadLock();
        try
        {
            return new ReadOnlyCollection<Character>(characters.ToArray());
        }
        finally
        {
            chrLock.ExitReadLock();
    ...
    public void moveMonster(Monster monster, Point reportedPos)
    {
        ...
        foreach (Character chr in getAllPlayers())
        {
    ...
    public void movePlayer(Character player, Point newPosition)
    {
        ...
        try
        {
            ...
            Dictionary<int, MapObject> mapObjects = getCopyMapObjects();
            foreach (MapObject mo in visibleObjects)
            {
                if (mo != null)
                {
        ...
        foreach (MapObject mo in getMapObjectsInRange(player.getPosition(), getRangedDistance(), rangedMapobjectTypes))
        {
    ...
    public void broadcastNightEffect()
    {
        ...
        try
        {
            foreach (Character chr in this.characters)
            {
                sendNightEffect(chr);
            }
        }
    ...
    public bool makeDisappearItemFromMap(MapObject? mapobj)
    {
    ...
    public class MobLootEntry : AbstractRunnable
    {
        ...
        private Monster mob;
        MapleMap _map;
        public MobLootEntry(MapleMap map, byte droptype, int mobpos, int chRate, Point pos, List<MonsterDropEntry> dropEntry, List<MonsterDropEntry> visibleQuestEntry, List<MonsterDropEntry> otherQuestEntry, List<MonsterGlobalDropEntry> globalEntry, Character chr, Monster mob)
        {
            _map = map;
    ...
    private class ActivateItemReactor : AbstractRunnable
    {
        ...
        public ActivateItemReactor(MapleMap map, MapItem mapitem, Reactor reactor, Client c)
        {
            _map = map;
    ...
    //{
    //    public Action<Client>? sendPackets { get; set; }
    //}
    ...
    //    public Func<Character, bool>? canSpawn;
    //}
    ...
    public void broadcastGMMessage(Character source, Packet packet, bool repeatToSource)
    {
    ...
    public void broadcastNONGMMessage(Character source, Packet packet, bool repeatToSource)
    {
        chrLock.EnterReadLock();
        try
        {
    ...
    {
        foreach (MapObject i in getMapObjectsInRange(player.getPosition(), double.PositiveInfinity, Arrays.asList(MapObjectType.ITEM)))
        {
            droppedItemCount.decrementAndGet();
            removeMapObject(i);
    ...
    {
        foreach (MapObject i in getMapObjectsInRange(new Point(0, 0), double.PositiveInfinity, Arrays.asList(MapObjectType.ITEM)))
        {
            droppedItemCount.decrementAndGet();
            removeMapObject(i);
    ...
    {
        List<Character> players = new(getCharacters());
    ...
    {
        List<Character> chars = new(getCharacters());
    ...
    public void startEvent(Character chr)
    {
    ...
    {   // ayy lmao
        ...
        {
            monsterDamaged = (Character from, int trueDmg) =>
            {
                ht.addHp(trueDmg);
            },
    ...
    public bool claimOwnership(Character chr)
    {
        if (mapOwner == null)
        {
            this.mapOwner = chr;
    ...
    public bool unclaimOwnership(Character? chr)
    {
    ...
    public bool isOwnershipRestricted(Character chr)
    {
    ...
    public void buffMonsters(int team, MCSkill skill)
    {
        ...
        foreach (MapObject mmo in getMapObjects())
        {
