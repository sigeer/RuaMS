...
namespace Application.Core.Game.Players
{
    public partial class Player
    {
        ...
        public async Task setQuestProgress(int id, int infoNumber, string progress)
        {
            ...
            await announceUpdateQuest(DelayedQuestUpdate.UPDATE, qs, false);
        ...
        public async Task awardQuestPoint(int awardedPoints)
        {
        ...
        public async Task raiseQuestMobCount(int id)
        {
            ...
            {
                await raiseQuestMobCount(MobId.GREEN_MUSHROOM_QUEST);
            }
            else if (id == MobId.ZOMBIE_MUSHROOM || id == MobId.ANNOYED_ZOMBIE_MUSHROOM)
            {
                await raiseQuestMobCount(MobId.ZOMBIE_MUSHROOM_QUEST);
            }
            ...
            try
            {
                foreach (QuestStatus qs in getQuests())
                {
                    ...
                    if (qs.getStatus() == QuestStatus.Status.COMPLETED || await qs.getQuest().canComplete(this, null))
                    {
                        continue;
                    }
        ...
        private async Task announceUpdateQuestInternal(Player chr, KeyValuePair<DelayedQuestUpdate, object[]> questUpdate)
        {
            ...
            switch (questUpdate.Key)
            {
                case DelayedQuestUpdate.UPDATE:
                    await SendPacket(QuestPacket.UpdateQuest(chr, (QuestStatus)objs[0], (bool)objs[1]));
                    break;

                case DelayedQuestUpdate.FORFEIT:
                    await SendPacket(QuestPacket.ForfeitQuest((short)objs[0]));
                    break;

                case DelayedQuestUpdate.COMPLETE:
                    await SendPacket(QuestPacket.CompleteQuest((short)objs[0], (long)objs[1]));
                    break;

                case DelayedQuestUpdate.INFO:
                    QuestStatus qs = (QuestStatus)objs[0];
                    await SendPacket(QuestPacket.UpdateQuestInfo(qs.getQuest().getId(), qs.getNpc()));
                    break;
            }
        }

        public async Task announceUpdateQuest(DelayedQuestUpdate questUpdateType, params object[] paramsValue)
        {
            ...
            {
                await announceUpdateQuestInternal(this, p);
            }
        }

        public async Task flushDelayedUpdateQuests()
        {
            ...
            {
                await announceUpdateQuestInternal(this, q);
            }
        }

        public async Task updateQuestStatus(QuestStatus qs)
        {
            ...
            if (qs.getStatus().Equals(QuestStatus.Status.STARTED))
            {
                ...
                await announceUpdateQuest(DelayedQuestUpdate.INFO, qs);
            }
            else if (qs.getStatus().Equals(QuestStatus.Status.COMPLETED))
            {
                ...
                {
                    await awardQuestPoint(YamlConfig.config.server.QUEST_POINT_PER_QUEST_COMPLETE);
                }
                qs.setCompleted(qs.getCompleted() + 1);   // Jayd's idea - count quest completed

                await announceUpdateQuest(DelayedQuestUpdate.COMPLETE, questid, qs.getCompletionTime());
        ...
        public async Task ClearExpiredQuests(long now)
        {
            ...
            {
                await expireQuest(item);
            }
        }

        public async Task expireQuest(Quest quest)
        {
            if (await quest.forfeit(this))
            {
                await SendPacket(QuestPacket.QuestExpire(quest.getId()));
            }
        }
        ...
        public async Task questTimeLimit(Quest quest, int seconds)
        {
            await SendPacket(QuestPacket.AddQuestTimeLimit(quest.getId(), seconds * 1000));
        }

        public async Task questTimeLimit2(Quest quest, long expires)
        {
            long timeLeft = expires - Client.CurrentServer.Node.getCurrentTime();

            if (timeLeft <= 0)
            {
                await expireQuest(quest);
            }
        }

        public async Task reloadQuestExpirations()
        {
