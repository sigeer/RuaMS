namespace Application.Core.Game.Commands.Gm3
{
    public class MapMobRateCommand : ParamsCommandBase
    {
        ...
        public override async Task Execute(IChannelClient client, string[] values)
        {
            ...
            await client.OnlinedCharacter.Notice($"全局的怪物倍率：x {client.OnlinedCharacter.getChannelServer().WorldMobRate}。当前地图的怪物倍率：x {curMap.MonsterRate}。当前地图实际倍率：x {curMap.ActualMonsterRate}");
        }
    }
}
