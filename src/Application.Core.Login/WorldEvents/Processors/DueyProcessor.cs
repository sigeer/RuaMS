...
namespace Application.Core.Login.WorldEvents.Processors;
...
public class DueyProcessor
{
    ...
    private static void showDueyNotification(ChannelClient c, Player player)
    {
        try
        {
            using var dbContext = new DBContext();
    ...
    private static int addPackageItemFromInventory(int packageId, ChannelClient c, sbyte invTypeId, short itemPos, short amount)
    {
        if (invTypeId > 0)
        {
    ...
    public static void dueySendItem(ChannelClient c, sbyte invTypeId, short itemPos, short amount, int sendMesos, string? sendMessage, string recipient, bool quick)
    {
    ...
    public static void dueyRemovePackage(ChannelClient c, int packageid, bool playerRemove)
    {
        if (c.tryacquireClient())
        {
            try
    ...
    public static void dueyClaimPackage(ChannelClient c, int packageId)
    {
        if (c.tryacquireClient())
        {
            try
            {
                try
                {
                    DueyPackageObject? dp = c.CurrentServer.Service.GetDueyPackage(packageId);
    ...
    public static void dueySendTalk(ChannelClient c, bool quickDelivery)
    {
        if (c.tryacquireClient())
        {
            try
            {
                ...
                {
                    c.sendPacket(PacketCreator.sendDuey(0x8, c.OnlinedCharacter.LoadMyDueyPackages()));
                }
            }
            finally
            {
